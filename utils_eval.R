...
process_fc_elim <- function(fcstate_fns, scen_code){
  fcsettings <- fix_fc_settings(scen_code)
  ## fcstate_fns include path
  ## sim, week, cases, S, E, I, A, R, N, incid, loglik
  map_dfr(1:length(fcstate_fns), function(i){
    par_id <- gsub(paste0("_seed", fcsettings$seed, ".csv"), "", gsub(paste0("GeneratedData/", fcsettings$mcode, "_", fcsettings$scode, "/fcstates_stoch_", fcsettings$str_if2, "_parid"), "", fcstate_fns[i]))
...
find_elimPeriod_novac <- function(simdata){
  ...
  returnData <- map_dfr(uqsims, function(uqsim){
    ...
    if (nrow(rleinit) < 52){ ## no elimination period dataset
      ...
      if(sum(testresurg$true_incid)>=(1/10000*mean(testresurg$pop))){ ## resurgence occurs
        print(paste(sum(testresurg$true_incid), (1/10000*mean(testresurg$pop))))
        ## find entire duration of elimination period (prior to resurgence)
        elim_beforeresurg_start <- min(rleinit$window_marker)
        elim_beforeresurg_end <- min(testresurg$window_marker)-1
        beforeresurg <- rleSimdata %>% dplyr::filter(window_marker %in% elim_beforeresurg_start:elim_beforeresurg_end)
...
process_fepi_plot <- function(fitstate_fns, hti_data){
  ...
  map_dfr(1:length(fitstate_fns), function(i){
    ...
    dplyr::summarise(cases_med = median(cmed), cases_lo = quantile(cmed, probs = c(.025)), cases_hi = quantile(cmed, probs = c(.975)), incid_med = median(imed), incid_lo = quantile(imed, probs = c(.025)), incid_hi = quantile(imed, probs = c(.975)), pop_med = median(pop_med)) %>%
    dplyr::full_join(hti_data %>% dplyr::rename(orig_cases = cases), by = c("week"))
}

process_fend_plot <- function(fitstate_fns, hti_data){
  ...
  map_dfr(1:length(fitstate_fns), function(i){
    ...
    ungroup %>%
    dplyr::mutate(week = week + 232) %>% # to endemic start week
    dplyr::rename(cmed = cases_med, imed = incid_med) %>%
    group_by(week) %>%
    dplyr::summarise(cases_med = median(cmed), cases_lo = quantile(cmed, probs = c(.025)), cases_hi = quantile(cmed, probs = c(.975)), incid_med = median(imed), incid_lo = quantile(imed, probs = c(.025)), incid_hi = quantile(imed, probs = c(.975)), pop_med = median(pop_med)) %>%
    dplyr::full_join(hti_data %>% dplyr::mutate(week = week_end + 232) %>% dplyr::select(-week_end) %>% dplyr::rename(orig_cases = cases), by = c("week")) %>%
    dplyr::arrange(week)
}
