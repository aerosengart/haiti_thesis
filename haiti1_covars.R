...
covars <- function(tmin, tmax, byt = 1, nbasis = 6, degree = 6, per = 52.14, dept = "Artibonite", data, settings) {
  haiti_dat <- data
  ndept <- settings$nd
  nweeks <- settings$nw
  ...
  ## number of cases in other depts
  ...
  noDept <- aggDat %>%
    dplyr::ungroup() %>%
    dplyr::select(-c(all_of(dept), date_sat, week))

  otherSum <- rowSums(noDept)
  other_cases <- data.frame(week = aggDat$week, other_cases = otherSum)
  ...
  covar_tab <- covar_tab %>%
    dplyr::left_join(other_cases, by = c("time" = "week"))
  ...
  covartab.all2 <- covartab.all %>%
    dplyr::select(time, contains("seas"), other_cases) %>%
